...
// Internal C struct matching C's Clay_ElementDeclaration memory layout
@(private)
_ElementDeclaration :: struct {
	layout:          LayoutConfig,
	backgroundColor: Color,
	overlayColor:    Color,
	cornerRadius:    CornerRadius,
...
// User-facing Odin ElementDeclaration struct including id
...
@(private)
_to_c_element_declaration :: proc(config: ElementDeclaration) -> _ElementDeclaration {
...
foreign Clay {
	_ConfigureOpenElement :: proc(config: _ElementDeclaration) ---
...
TextStatic :: proc($str: string, config: TextElementConfig) {
	wrapped := MakeString(str)
	wrapped.isStaticallyAllocated = true
	_OpenTextElement(wrapped, config)
}

TextDynamic :: proc(str: string, config: TextElementConfig) {
	_OpenTextElement(MakeString(str), config)
}
...
ToSlice_Val :: proc(array: ClayArray($T)) -> []T {
	return array.internalArray[:array.length]
}

ToSlice_Ptr :: proc(array: ^ClayArray($T)) -> []T {
	return array.internalArray[:array.length]
}
